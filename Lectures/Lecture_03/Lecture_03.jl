...
begin 
	...
	<div align=center style="font-size:25px; font-family:family:Georgia">UD/ISCTE-IUL Trading and Bloomberg Program</div>
	<p style="padding-bottom:1cm"> </p>
...
begin
	...
	<fieldset>      
		...
		<br>
        <input type="checkbox" value="">Understand how prices for Treasury securities are quoted in secondary markets.<br>
	<br>
	<input type="checkbox" value="">Know what a discount yield is and how it is used to calculate prices of Treasury bills. 
	<br>
	<br>
	<input type="checkbox" value="">Know how to calculate accrued interest for Treasury notes and bonds.
	<br>
	<br>
	</fieldset>      
	"""
...
# ╔═╡ 899f8d41-b739-490e-bd8a-ed74ed458307
Foldable("Bloomberg Word Bond Markets",LocalResource("BloombergWordBondMarkets.png"))

# ╔═╡ a61e8501-3a36-4d9b-8a8c-daf9d9eea50c
Foldable("...",LocalResource("BING_01.jpg"))
...
- On the Bloomberg Terminal type `912797HP5` and click on the security in the search result. 
...
LocalResource("./TreasuryBillPriceQuoteBloomberg.png",:width => 500) 
...
md"""
- Treasury bill prices are quoted as yields on a **bank discount basis**.
- For example, in the screenshot above, the number **4.8100** in the first row of the table means a discount yield of 4.8100 percent.
- What do we have to pay in dollar terms for this Treasury bill?
"""
...
# ╔═╡ d48b7ae1-8844-4659-92fc-7d348d49c5fc
md"""
- This is not difficult to figure out, since there is a simple formula we can use
"""
...
- __Note 1: $y_d$ above is a decimal number, i.e. we need to use 0.048100 if the discount yield is 4.8100 percent.__
- __Note 2:__ _Days to Maturity_ is the number of days until the maturity date of the Treasury bill.
- __Note 3:__ The price $P$ is the dollar amount we pay for a Treasury bill with face value of __\$100__.
"""

# ╔═╡ 63eca197-3246-4758-be7b-998d1afebdd0
vspace

# ╔═╡ 5e1214b5-1856-46d6-90e5-d75171a39557
html"""
	<div style="text-align: right;margin-top:1cm;"> Page 5</div>
"""

# ╔═╡ 848328ba-32f3-44e5-ba16-da95bae8ef92
md"""
### Treasury Bill Price Quote - Example 1
- Let's now apply this formula to calculate the price of the Treasury bill with a discount yield of _4.8100 percent_. Assume that the Treasury bill has its maturity date in _355 days_ from today.
- What is the price of this Treasury bill (for \$100 par value investment)?

"""

# ╔═╡ f2e2af0e-c850-46ef-990a-d18b0af4448e
Foldable("Solution",md"""
- The price is $$P=100 \times \left( 1- y_d \times \frac{\textrm{Days to Maturity}}{360}\right)= 100 \times \left( 1- 0.0481 \times \frac{355}{360}\right)=\$95.25681$$
- This means we pay \$95.26 today for a Treasury bill with face value of \$100. At maturity in 355 days, we receive back \$100 on our investment.
""")

# ╔═╡ 077abe6c-36e5-49a8-a852-2b5dfa769548
vspace

# ╔═╡ 3d7bd9f8-7659-4265-aaf5-fcd2ad56e829
html"""
...
md"""
### Daycount Convention for Treasury Bills
...
# ╔═╡ 0698baee-0c77-485d-a9f6-5d8a2ecbdc2d
vspace

# ╔═╡ 674b8aae-c8f1-4586-a2e9-1fad2c58e86e
html"""
	<div style="text-align: right;margin-top:1cm;"> Page 7</div>
"""

# ╔═╡ f8d0cd5c-7378-4628-acd1-d8dc334c0b8a
md"""
### Treasury Bill Price Quotes - Example 2
"""
...
# ╔═╡ 9931408b-da74-438c-80d6-35b189d97e81
md"""
- Quote (as discount yield) on 2/07/2020.
- Assume that we buy this Treasury bill on 2/10/2020.
- How many days are there until the maturity date?
"""

# ╔═╡ 513e8c93-9996-43cd-992a-c12742ad3e29
Foldable("Solution",md"""
- There are 80 days from today to maturity.
...
- Note: these are expressed as percentages, i.e. 1.5225 means 1.5225 percent.
"""
...
Foldable("Solution",md"""
$$P=100 \times \left( 1- y_d \times \frac{\textrm{Days to Maturity}}{360}\right)= 100 \times \left( 1- 1.5225\% \times \frac{80}{360}\right)=\$99.6617$$
- This means that we pay \$99.6617 to buy \$100 face value of this Treasury bill. At maturity on 4/30/2020, we receive \$100 on this investment.
""")
...
md"""
- Let's verify that our calculation is correct.
"""
...
md"""
### Treasury Bill Price Quotes - Example 3
"""
...
# ╔═╡ 9b1950c3-3e08-4034-834b-08a48834f474
vspace

# ╔═╡ de6dd4bc-b9f4-4413-925e-d09d831cc78d
html"""
...
Markdown.parse("
- What is the purchase price of the Treasury bill (per \$100 par value)?
")

# ╔═╡ b3e8ed37-4d75-42dd-ae6b-0cf8d78a6425
md"""
- What is the purchase price for \$1000 par value?
"""
...
- The Treasury Bill is priced for **settlement** on $(Date(settle_4)).
  - The so-called __settlement date__ is the date upon which you own the Treasury bill. Typically, when you buy a Treasury bill today, you own it on the next day. Thus, the settlement date is one day after the trade date. 
  - To calculate the number of days to maturity, we count the days between the settlement date and the maturity date.
- Calculate the price of the Treasury Bill.
""")

# ╔═╡ e3045b79-b622-4cdc-9e8d-01f4bf94f2a6
Foldable("Solution",Markdown.parse("
- We need to count the number of days between the settlement date ($(Date(settle_4))) and the maturity date ($(Date(mat_4))).  
- Counting the days gives us $(daysMat) days to maturity:
"))

# ╔═╡ 728b97ce-2e55-4bf0-b6d8-c131eb134a15
Foldable("Solution",
Markdown.parse("
- Thus, this Treasury bill has a dollar purchase price of 
     ``P=100\\times\\left(1 - y_d \\times \\frac{\\textrm{Days to Maturity}}{360}\\right)=\\\$100\\times \\left(1 - $yd_4\\% \\times \\frac{$(Dates.value(daysMat))}{360} \\right) = \\\$ $(roundmult(px_4,1e-6))`` per \$ $(par_4) face value.
")
)

# ╔═╡ ba63e285-8def-4f63-b0a4-163a43804808
Foldable("Solution",
Markdown.parse("
- For a face value of \\\$1000, we need to pay \$ `` $(roundmult(10*px_4,1e-4))``.
")
)
...
# ╔═╡ 0b0e531e-c0fb-43ae-92fe-b3d0c5329038
vspace
...
- What is the dollar price (assuming a \$100 par value investment)?
")

# ╔═╡ a468e100-7351-44bf-9858-fc630e81c8c4
Foldable("Solution",
Markdown.parse("
...
# ╔═╡ b3a75eb7-2ece-4bbf-b179-4f43c42a9c15
html"""
...
md"""
# Accrued Interest on Treasury Notes/Bonds
"""

# ╔═╡ 6d32dfe2-9377-4d43-8ada-d9b3a47bc15e
md"""
- Let's start with the following example of a 5-year Treasury note.
"""
...
# ╔═╡ 72e93779-fb3f-4872-bbeb-28414b5e4aa0
html"""
	<div style="text-align: right;margin-top:1cm;"> Page 22</div>
"""

# ╔═╡ 2abfd840-d751-4b5a-95d9-d9c731866e8b
md"""
- Let's look at the following price quotes
- Notice that there are two prices. The second price is labeled "Dirty".
- Let's understand what this means.
"""

# ╔═╡ a7c9120b-ee24-48c2-904a-e40ef95fcffa
LocalResource("./TreasuryNoteDescrExampleBloombergDirtyPrice.png",:width => 600) 

# ╔═╡ 6ce18679-b433-4110-8ef4-0fd9135737f4
Foldable("Bloomberg Instruction",md"""
> - How to get there on the Bloomberg terminal?
>   - On the keyboard type `T Govt`
>   - Click on `United States Treasury Note/Bond (Multiple Matches) Govt`
>   - On the next page, click on one of the Treasury notes/bonds.
...
# ╔═╡ 8e332489-0b65-40b6-8c4e-a8305c75d076
vspace
...
# ╔═╡ 20d08d8a-de07-4aae-9632-a0ebb5086572
md"""
- To understand the concept of a **"Dirty Price"**, let's look at the coupon cash flows of this Treasury note.
- When we own a Treasury note/bond, we receive the full coupon cash flow every six months.
  - In this example, the Treasury has a coupon rate of 0.875 percent.
  - This means, the Treasury pays every six months on a face value of \$100 a coupon cash flow of

$$C = \frac{0.00875}{2} \times \$100 = \$0.4375$$
- The Bloomberg screenshot below shows the coupon cash flows for \$1M.
  - This is easy to calculate $\frac{1,000,000}{100} \times 0.4375 = 4,375$
"""

# ╔═╡ f0ad48c3-501c-4c94-8584-3539df25de1f
LocalResource("./TreasuryNoteCashflowExampleBloomberg.png",:width => 1200) 

# ╔═╡ 8bf6a21e-38fa-42af-96d9-577896add78a
html"""
	<div style="text-align: right;margin-top:1cm;"> Page 24</div>
"""

# ╔═╡ 9a8eca3d-f157-4a8f-9981-b19c9e22c50c
md"""
- In general, the coupon cash flows of a Treasury note look as in the following graph.
"""

# ╔═╡ 12144276-7937-4e18-9bf0-c059cf73bad8
md"""
!!! hint ""
- Coupon rate $c$ [percent]: $(@bind C Slider(0:0.1:20.0, default=10, show_value=true))
- Maturity $T$ [years] : $(@bind T Slider(1.0:1:10.0, default=5.0,show_value=true))
"""

# ╔═╡ 0c3f1ac3-6fa7-4a86-ac45-9416440eeca7
let
	CF = 0.5*C.*ones(convert(Int64,T*2))
	CF[end] += 100
	dt = collect(0.5:0.5:T)
	bar(dt,CF,label="", ylim=(0,110), yticks=collect(0:5:110), xlim=(0,T+1), 
	bar_width = 0.1,
	xticks=collect(0.0:0.5:T), xlabel="Years", ylabel="Coupon Cash Flow")
end

# ╔═╡ b115f762-6203-4289-9e70-ef4112c4a82d
html"""
	<div style="text-align: right;margin-top:1cm;"> Page 25</div>
"""

# ╔═╡ 00c4b429-e594-401b-aff3-c8803499c6b8
md"""
- Now, suppose we buy this Treasury three months after it was issued.
- This means that we own the Treasury at $t=0.25$ (three months), indicated by the red arrow.
- Since we own the Treasury when the first coupon is paid at $t=0.5$, we collect the full coupon cash flow.
- However, we only owned it for three months (from $t=0.25$ to $t=0.5$).
- In other words, there are six months between coupon cash flows, and we owned it only for three months.
- Thus, when we buy the Treasury note, we pay to the seller of the Treasury half of the coupon cash flow.
- This amount is referred to as __"accrued interest."__
"""

# ╔═╡ 9ecd5bf5-5e74-4842-b881-e604f25b39dc
let
	CF = 0.5*C.*ones(convert(Int64,T*2))
	CF[end] += 100
	dt = collect(0.5:0.5:T)
	...
	bar_width = 0.1,
	xticks=collect(0.0:0.5:T), xlabel="Years", ylabel="Coupon Cash Flow")
	plot!([0.25,0.25],[0,10],arrow=true,color=:red,linewidth=4,label="")
end

# ╔═╡ 5efbc727-bb1a-4423-9d6a-816ca6fbf514
vspace
...
# ╔═╡ 9ed5a220-6277-4c4f-8efa-26ca701b9234
vspace

# ╔═╡ 3575e080-93e3-4e3d-a679-07aafc1659b7
html"""
...
# ╔═╡ 4c6ead94-da6c-4310-a8d2-f7a1e6be213c
md"""
- Interest accrues between the last and the next coupon cash flow.
  - The dates of the last and the next coupon cash flows matter for the calculation of accrued interest.
- Accrued interest is an amount between zero and the full coupon cash flow amount. 
"""
...
- Coupon rate [% p.a.]: $(@bind cpnAI_1 Slider(0:1:10, default=4, show_value=true))
...
Markdown.parse("""
- Suppose a Treasury note has a coupon rate of $(cpnAI_1) % (paid semi-annually) and that there are 180 days between coupon payments.
- Suppose we **own** \$ 100 par value of this Treasury note and that $(daysAI_1) days have passed since the previous coupon cash flow.
...
Markdown.parse("""
- Now, suppose we decide to sell the Treasury note.
- When you sell the Treasury note, we no longer receive the coupon cash flow in $(180-daysAI_1) days. 
- The buyer receives the full coupon interest of \$ $((cpnAI_1/100)/2*100).
- However, we owned the bond for $(daysAI_1) out of the 180 days between coupon interest cash flows.
- Thus, we should be entitled to receive a fraction equal to \$\\frac{$daysAI_1}{180} = $(roundmult(daysAI_1/180,1e-2))\$ of the total \$ $((cpnAI_1/100)/2*100) coupon interest. This amount of \$ $(roundmult(daysAI_1/180*((cpnAI_1/100)/2*100),1e-2)) is referred to as __accrued interest__.
""")
...
md"""
- We now understand the concept of accrued interest. Let's now turn to how it is calculated in practice.
- To calculate accrued interest, the following are needed: 
  ...
  - the **number of days in the coupon period** (represents the number of days between the last and the next coupon payment)
...
- Accrued Interest is then calculated as
$$\textrm{Accrued Interest} = \textrm{Coupon Interest Cash Flow} \times \left( \frac{ \textrm{\color{red}Days in Accrued Interest Period}}{\textrm{\color{blue}Days in Coupon Period}} \right)$$
"""
...
md"""
### 1. Calculate **Days in Accrued Interest Period**
...
# ╔═╡ b9bdb5fb-4d24-4bcb-af41-af12eae37bec
html"""
...
$$\textrm{\color{red} Days in Accrued Interest Period=}$$ 
...
md"""
### 2. Calculate **Days in Coupon Period**
- Simply the number of days between the previous coupon date and the next coupon date.
"""

# ╔═╡ 4c7b7b5a-9736-4dda-883e-9fccad6055ec
md"""
!!! terminology "Days in Coupon Period"
$$\textrm{\color{blue} Days in Coupon Period=}$$ 
$$\textrm{Days between the last and the next coupon cash flow date.}$$
"""
...
- Suppose we buy Treasury security with a settlement date of September 10.
...
**Step 1**: *Days in Accrued Interest Period*
  - We count the actual number of days between May 15 (the previous coupon date) and September 10 (the settlement date). What do we get?
"""

# ╔═╡ 67a43546-4134-4827-8d15-b33b3959dc6c
Foldable("Solution",
md"""
- We count as follows:
...
- We count the actual number of days between May 15 (the previous coupon date) and Nov 15 (the next coupon date). What do we get?
"""

# ╔═╡ 8000a8ce-3a2f-42ec-a47c-86023f71e557
Foldable("Solution",
md"""
- We get
   - May: **16 days**
...
- What is the accrued interest (per \$100 par value)?
"""

# ╔═╡ 92b1bc76-a0ea-4fc3-af98-3795288b8435
Foldable("Solution",
md"""
##
...
- Note: this is the accrued interest for \$100 face value.
""")

# ╔═╡ b32585ce-1455-4129-ba58-f0a00b9b1c3a
vspace

# ╔═╡ dc86f9f7-e4d3-4e00-ac9e-2b4ed54711d9
html"""
...
# ╔═╡ 485f5d32-6a63-45b1-85ad-822462226e8a
md"""
### Daycount Convention Example
"""
...
- The number of days between May 15 and July 17 is 62 days, 
  ...
  - So the total number of days is 16 days + 30 days + 16 days = 62 days.
"""
...
begin
	...
	<fieldset>      
        <legend>Goals for today</legend>      
		<br>
        <input type="checkbox" value="" checked>Understand how prices for Treasury securities are quoted in secondary markets.<br>
	<br>
	<input type="checkbox" value="" checked>Know what a discount yield is and how it is used to calculate prices of Treasury bills. 
	<br>
	<br>
	<input type="checkbox" value="" checked>Know how to calculate accrued interest for Treasury notes and bonds.
	<br>
	<br>
	</fieldset>      
	"""
...
# ╔═╡ f98efdaf-672a-44c0-8984-3519482f4f4c
html"""
	<div style="text-align: right;margin-top:1cm;"> Page 48</div>
"""

# ╔═╡ 33e70b30-6254-45b8-8ca9-c470aa20f4d3
md"""
# Study Guide
- How are Treasury bills quoted in the financial markets? Do you know what a discount yield is? Can you calculate the price of a Treasury bill in dollars per \$100 par value if a broker gives you a quote in terms of a discount yield? What is a daycount convention and what daycount convention is used for Treasury bills?
  - [Review](#a81f9bd5-374d-4238-af83-e39ab1f5982e)
- Can you calculate the price of a Treasury note per \$100 par value if a broker gives you a price quote in points, 32nds, and 64ths? 
  - [Review](#93db6880-429c-4b9c-a807-eba600e03df1)
- Can you calculate the  coupon cash flows on a Treasury note if you are given a coupon rate? Can you determine the dates on which coupons are paid if you are given the maturity date of a Treasury note?
  - [Review](#93db6880-429c-4b9c-a807-eba600e03df1)
- What is accrued interest? Can you calculate the accrued interest of a Treasury note? How large can the accrued interest of a Treasury get?
  - [Review](#be1e2ae2-8b05-42c7-bc54-18c9ff111854)
- What is a settlement date, a trade date, and a maturity date?
  - [Review](#be1e2ae2-8b05-42c7-bc54-18c9ff111854)
- What is a dirty price and what is a clean price? When is the dirty price equal to the clean price?
  - [Review](#be1e2ae2-8b05-42c7-bc54-18c9ff111854)
- What is the purpose for daycount conventions? Can you name two different types of daycount conventions used in the fixed income markets? What is the daycount convention used for Treasury notes?
  - [Review](#70661dd7-0acf-4b6c-b7dd-f4ad71c1cee9)
"""

# ╔═╡ 3fb82ae0-1c6a-47c7-9f38-d9d890dfa9c5
vspace

# ╔═╡ 06f7c9f6-f3e9-45f7-80f7-3ef40c59f40e
html"""
...
manifest_format = "2.0"
project_hash = "3217bdfeec65a279a3418625ce4d67e3a5c13ff5"
